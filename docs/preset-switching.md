...
1. Define named presets in `sylastra-agent-tree.jsonc` under the `presets` field
...
| Method | How | Persists? |
|--------|-----|-----------|
| Config file | Set `"preset": "cheap"` in `sylastra-agent-tree.jsonc` | Yes, across restarts |
