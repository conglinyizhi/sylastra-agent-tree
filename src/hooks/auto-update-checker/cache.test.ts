  ...
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest/node_modules/sylastra-agent-tree/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest/package.json',
      });
      ...
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/sylastra-agent-tree@latest/node_modules/sylastra-agent-tree',
      );
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/sylastra-agent-tree'),
      );
      const readSpy = spyOn(fs, 'readFileSync').mockReturnValue(
        JSON.stringify({
          dependencies: {
            'sylastra-agent-tree': '1.0.1',
          },
        }),
      );
      const writeSpy = spyOn(fs, 'writeFileSync').mockImplementation(() => {});
      ...
      const result = preparePackageUpdate('1.0.1', 'sylastra-agent-tree', null);
