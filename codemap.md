...
`sylastra-agent-tree` is an OpenCode plugin that adds a specialist-agent operating model on top of the host runtime. Its core job is to:
...
| `scripts/generate-schema.ts` | Generates `sylastra-agent-tree.schema.json` from the Zod config schema. |
