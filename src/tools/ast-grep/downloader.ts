...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'sylastra-agent-tree', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[sylastra-agent-tree] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[sylastra-agent-tree] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[sylastra-agent-tree] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[sylastra-agent-tree] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
